              ...
              nodes=FALSE, cutoff, ...)
{
    ...
    mdim <- ncol(x)
    ntest <- nrow(x)
    ntrain <- length(object$predicted)
    ...
    proxmatrix <- if (proximity==1) matrix(0, ntest, ntest) else if(proximity==2) matrix(0,ntest,ntrain) else numeric(1)
    nodext <- if (proximity==2) object$nodes else NULL
    if (proximity==2 && is.null(nodext)) {
      stop("No nodes were found.")
    }
    ...
    if (object$type == "regression") {
            ...
            ans <- .C("regForest",
                  ...
                  nodext = as.integer(object$nodes), # different than RF
                  ntrain = as.integer(ntrain), # different than RF
                  DUP=TRUE,
                  PACKAGE = "generalizedForest")[keepIndex]
