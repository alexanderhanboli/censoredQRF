...
one_run <- function(n, n_test, tau) {
  ...
  nodesize <- 8
  ntree <- 1000
  Yc <- crf.km(y ~ x, ntree = ntree, nodesize = 3*nodesize, data_train = data_train, data_test = data_test, 
               yname = 'y', iname = 'ind', 
               tau = tau)
