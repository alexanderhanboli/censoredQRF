...
ntree <- 1000
tau <- 0.7
quantile_result <- list('nodesize'=rep(NA,B), 'crf'=rep(NA,B), 'qrf'=rep(NA,B), 'grf'=rep(NA,B), 
                        'qrf_oracle'=rep(NA,B), 'grf_oracle'=rep(NA,B))
cindex_result <- list('nodesize'=rep(NA,B), 'crf'=rep(NA,B), 'qrf'=rep(NA,B), 'rsf'=rep(NA,B), 'grf'=rep(NA,B),
                      'qrf_oracle'=rep(NA,B), 'grf_oracle'=rep(NA,B))
for (t in 1:B) {
  if (t%%20 == 1) {
    nodesize <- nodesize + 5
...
ggsave(paste0(dataset, "_quantile_loss_nodesize_", 10*tau, ".pdf"), width = 5, height = 5, path = "./results/")
...
ggsave(paste0(dataset, "_cindex_nodesize_", 10*tau, ".pdf"), width = 5, height = 5, path = "./results/")