...
one_run <- function(b, data, xnam, ynam, inam, tau, nodesize, ntree) {
  ...
  Yc <- crf.km(fmla, ntree = ntree, nodesize = 6*nodesize, data_train = data_train, data_test = data_test, 
  ...
  qrf <- quantregForest(x=data_train[,xnam,drop=FALSE], y=data_train[,ynam], nodesize=6*nodesize, ntree=ntree)
...
ntree <- 2000
tau <- 0.5
quantile_result <- list('crf'=rep(NA,B), 'qrf'=rep(NA,B))
