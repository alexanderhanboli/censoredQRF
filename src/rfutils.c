...
/* Compute proximity with train or test. */
...
*/
    int i, j;
    /* add function to count the number of training samples in a node */
    double *nodeNum;
    nodeNum = (double *)Calloc(n, double);
    // count the number of points in each training terminal node
    for (i = 0; i < n; ++i) {
      nodeNum[i] += 1.0;
      for (j = i + 1; j < n; ++j) {
        if (node[i] == node[j]) {
        ...
        for (j = i+1; j < n; ++j) {
            if (oobprox) {
                if (! (inbag[i] > 0 || inbag[j] > 0) ) {
                    ...
                    oobpair[i*n + j] ++;
                    if (node[i] == node[j]) {
                        prox[j*n + i] += 1.0/nodeNum[j];
                        prox[i*n + j] += 1.0/nodeNum[j]; // nodeNum[i] = nodeNum[j]
                    }
                }
            } else {
                if (node[i] == node[j]) {
                    prox[j*n + i] += 1.0/nodeNum[j];
                    prox[i*n + j] += 1.0/nodeNum[j];
                }
            }
        }
    }
    ...
    for (i = 0; i < n; ++i) {
      prox[i*n + i] += 1.0/nodeNum[i];
    }

    Free(nodeNum);
}

/* Compute proximity between train and test. */
void computeProximityCross(double *prox, int *nodea, int *nodeb, int m, int n) {
/* Accumulate the number of times a pair of points fall in the same node.
   prox:    m x n proximity matrix
   nodea:  vector of terminal node labels from A (test)
   nodeb:  vector of terminal node labels from B (train)
   m:       total number of cases in test
   n:       total number of cases in train
*/
    int i, j;
    double *nodeNum;
    nodeNum = (double *)Calloc(n, double);
    ...
    for (i = 0; i < n; ++i) {
      ...
      // Note that in this way, nodeNum[i] >= 1 for sure.
    }
    // Calculate the proximity matrix between test and train data.
    // The weights now are 1/#points in each node.
    for (i = 0; i < m; ++i) {
        for (j = 0; j < n; ++j) {
              if (nodea[i] == nodeb[j]) {
                  prox[i + j*m] += 1.0/nodeNum[j];
              }
        }
    }
