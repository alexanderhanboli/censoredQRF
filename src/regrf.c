...
void regForest(double *x, double *ypred, int *mdim, int *n,
               ...
               double *proxMat, int *nodes, int *nodex,
						   int *nodext, int *ntrain) {
    int i, j, idx1, idx2, idx3, *junk;
    double *ytree;
    ...
    if (*doProx) {
			if (*doProx > 1) zeroDouble(proxMat, *n * *ntrain);
			else zeroDouble(proxMat, *n * *n);
		}
    ...
    for (i = 0; i < *ntree; ++i) {
			...
			if (*doProx) {
				if (*doProx > 1) computeProximityCross(proxMat, nodex + idx2, nodext + idx3, *n, *ntrain);
				else computeProximity(proxMat, 0, nodex + idx2, junk,
						      junk, *n);
			}
